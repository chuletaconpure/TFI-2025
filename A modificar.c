...
//CRUD preguntas
void crearPregunta(struct lista1 **L);
void mostrarPreguntas(struct lista1 *L,int preguntaId);
void modificarPregunta(struct lista1 *L,int preguntaId, int encuestaId);
void eliminarPregunta(struct lista1 **L,int preguntaId, int encuestaId);
...
int main(){
    ...
    struct lista1 *L1=NULL,*nodoL1=NULL;
	...
	while(!apagado){
		...
		if(w==13){
			...
			if(select==6){
				system("cls");
				int idPregunta;
				printf("Ingrese el id de la pregunta a mostrar: ");
				fflush(stdin);
				scanf("%d",&idPregunta);
				mostrarPreguntas(L1,idPregunta);
				getch();
			}
			if(select==7){
				system("cls");
				int idPregunta,idEncuesta;
				printf("Ingrese un id de Pregunta y de encuesta correspondiente: ");
				scanf("%d",idPregunta);
				scanf("%d",idEncuesta);
				modificarPregunta(L1,idPregunta,idEncuesta);
				getch();
			}
			if(select==8){
				eliminarPregunta(&L1,1,1);
			}
			if(select==9){
				apagado++;
            }
...
void crearPregunta(struct lista1 **L){
	struct lista1 *nueva = NULL;
	if(nuevoL1(&nueva)){
		printf("Ingrese el Id de la encuesta: ");
		scanf("%d", &nueva->Encuesta_Id);
		printf("Ingrese Id de la pregunta: ");
		scanf("%d", &nueva->Pregunta_Id);
		printf("Ingrese la pregunta: ");
		fflush(stdin);
		scanf("%s", &nueva->Pregunta);
		printf("Ingrese la ponderacion de la pregunta");
		scanf("%f",&nueva->Ponderacion);
		nueva->sgte = NULL;
		
		insertarL1(&nueva, L);
		printf("Pregunta creada con exito. \n");
	}else{
		printf("Error al reservar el memoria para la respuesta. \n");
	}
}

void mostrarPreguntas(struct lista1 *L,int preguntaId){
	printf("Pregunta con el id %d: ", preguntaId);
	while(L !=NULL){
		if(L->Pregunta_Id == preguntaId){
			printf("Encuesta ID: %d | PreguntaId: %d | Pregunta: %s | Ponderacion: %.2f",L->Encuesta_Id,L->Pregunta_Id,L->Pregunta,L->Ponderacion);
		}
		L = L->sgte;
	}
}
...
void modificarPregunta(struct lista1 *L,int preguntaId, int encuestaId){
	...
	while(L != NULL){
		if(L->Encuesta_Id == encuestaId && L->Pregunta_Id == preguntaId){
			printf("\nPregunta actual: %s",L->Pregunta);
			printf("\nIngrese la pregunta: ");
			fflush(stdin);
			scanf("%s", &L->Pregunta);
			printf("\n¿Desea modificar la ponderacion?\n1-Si\n2-No\n ");scanf("%d",&modificar);
			while(modificar > 2 || modificar <  1){
				printf("Ingrese bien la opcion \n1-Si\n2-No\n");scanf("%d",&modificar);
			}
			if(modificar==1){
				printf("\nIngrese la nueva ponderación: ");
				scanf("%f", &L->Ponderacion);	
			}
			printf("\nPregunta modificada.\n");
	...
	printf("ID de Encuesta o Pregunta no encontrada\n");
}

void eliminarPregunta(struct lista1 **L,int preguntaId, int encuestaId){
	struct lista1 *actual = *L;
	struct lista1 *anterior = NULL;
	
	while(actual != NULL){
		if(encuestaId == actual->Encuesta_Id && actual->Pregunta_Id == preguntaId){
			struct lista1 *aBorrar = actual;
			if(anterior == NULL){
				*L = actual->sgte;
				actual = *L;
			}else{
	...
	printf("Pregunta eliminada y sus respuestas correspondientes tambien.\n");
}
